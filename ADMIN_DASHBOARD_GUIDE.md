...
> 2026-07-27 추가, 2026-08-28 3개 앱 백업 상태 갱신. 선생님 계정이 늘어나면서 "누가 실제로 쓰고 있는지",
...
| 💾 **백업 상태** | 아지트·샘링크·자비스의 앱별 백업/복구, 세 공용 사본, 최근 20건 이력 |
...
### 🧭 첫 화면 — 지금 확인할 일 · 오늘 이용 현황 · 시스템 상태

관리자로 들어오면 어느 묶음보다 먼저 보이는 요약입니다. 세 구역으로 나뉩니다.

| 구역 | 무엇이 보이나 |
|---|---|
| **지금 확인할 일** | 승인 대기·장기 미접속·정리 대상처럼 **사람이 손대야 하는 것** |
| **오늘 이용 현황** | 오늘 접속 교사 · 오늘 접속 학생 · 오늘 제출글 (2026-08-27 신설) |
| **시스템 상태** | 서버·백업·장애 알림 |

`오늘 이용 현황`의 기준은 **한국 시간 0시부터 지금까지**입니다. 화면에도 같은 문구가 적혀 있습니다.
어제까지의 흐름이나 최근 7일 추이를 보려면 숫자를 눌러 `📈 서비스 현황`으로 넘어갑니다.

> 여기 숫자는 확인에서 끝나지 않습니다. 모든 항목이 실제 처리 화면으로 이동합니다.
...
보는 것: 앱 응답 · DB 응답 · 디스크 여유(10GB) · 컨테이너 상태 · 백업/복구 지연 ·
최신 실행의 앱 결과 3행 누락/실패.
...
- 관리자 첫 화면의 `앱 백업` 카드는 최신 통합 백업이 아지트·샘링크·자비스 **3/3 정상**인지
  보여 주며, 누르면 이 화면으로 바로 옵니다.
- 매일 오전 4시 백업이 끝나면 앱별 DB·파일/설정 결과와
  `내장 / 암호화 Drive / 외장 SSD`, 필수 산출물 7개 상태가 기록됩니다.
- 매월 1일 오전 4시 40분 실제 복구 리허설이 끝나면 같은 3개 앱의 DB 표와
  Storage·설정·압축 파일 복원 결과가 기록됩니다.
- 앱 카드는 정상이어도 공용 사본이나 실제 복구가 실패할 수 있으므로 두 구역을 따로 확인합니다.
- 앱별 기록 기능을 붙이기 전 과거 실행은 억지로 추정하지 않고 `앱별 기록 전`으로 보입니다.
