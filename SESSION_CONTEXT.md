...
- ⚠️ 2026-08-17 덱마스터 **화면 배포가 GitHub 429(요청 과다)로 막혔다.** DB는 적용됐고(155/155) 화면만
  안 올라간 상태다. 앱은 정상이고 기존 동작에 영향은 없다. 재시도로 풀리며, **하루에 배포를 열 번 넘게
  돌리면 이 제한에 걸리므로 변경을 묶어 한 번에 배포한다.**
...
4. 교사 실계정에서 어휘의 탑 관리 화면에 V1 선택이 없고 학년 변경 시 현재 덱 안내가 함께 바뀌는지,
   일반 PC 해상도에서 설정 저장 버튼까지 스크롤 없이 보이는지 확인한다.
5. 운영 Google Cloud의 Drive API 활성화를 확인하고, 교사 실계정으로 WebP 보고서를 Excel·Google Docs로
   내보내 사진 위치와 Google Drive 임시 파일 삭제를 확인한다.
6. 학생 태블릿 보고서 전체 동선, 교사 PDF 두 양식, 맞춤법 500개 검색·랜덤 5문제를 확인한다.
7. 다음 04:00 백업 뒤 새 `아지트Storage.tar.gz`가 내장·암호화·외장 사본에 포함됐는지, 백업 로그에 `✓ 알림정리`가 남는지 확인한다.
8. `docker system df`로 빌드 캐시를 확인한다. 도커 VM 디스크는 31GB뿐이라 캐시가 차면 Kong 건강검진 실패와
