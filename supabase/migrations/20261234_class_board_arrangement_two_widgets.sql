-- 우리 반 스크린: 자리·역할 배치 위젯을 둘까지 (2026-09-03)
--
-- 왜: 교사가 "자리 배치와 역할 배치를 **둘 다** 띄우고 싶은데 하나만 넣힌다"고 알려 왔다.
-- 화면과 서버가 모두 하나로 막고 있었다. 종류가 자리·역할 둘뿐이므로 둘까지 받는다.
--
-- ⚠️ 화면(`manifest.js` 의 `maxInstances`)과 여기 숫자가 **같아야** 한다.
--    한쪽만 늘리면 화면에서는 넣히는데 저장할 때 막힌다.
--
-- 아래는 20261233 의 검증 함수를 그대로 두고 개수만 고친 것이다(손으로 옮겨 적지 않았다).
