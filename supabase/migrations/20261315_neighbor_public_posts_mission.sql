-- 공개 글 관리에서도 주제별로 묶어 보고 일괄 비공개할 수 있게, 공개 글(public_posts)에
-- 과제(주제) 정보를 함께 실어 준다(2026-09-18).
--
-- 지금까지 public_posts 는 제목·발췌·작성자만 담아, 화면에서 학생 이름으로는 묶을 수 있어도
-- 주제로는 묶을 수 없었다. 원글의 mission_id·제목을 LEFT JOIN 으로 붙인다(자율 글은 '자율 글').
-- 코어 빌더 하나만 통째로 다시 만든다(다른 부분은 20261256 과 같다 — public_posts 두 줄만 추가).
