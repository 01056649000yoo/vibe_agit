-- 학급 발자국에 일기를 유형별로 담는다.
--
-- 완료 판정을 `writing_counts_as_completed()` 로 통일하고, 자율 글을 `reading_log` 하나로
-- 뭉뚱그리던 post_type 을 실제 유형(self_writing_type)으로 살린다. 그러지 않으면 일기가
-- 독서록 편수에 섞여 교사 화면이 사실과 다른 숫자를 보여 준다.
-- 출력에 `diaries` 를 더한다. 기존 `reading_logs`·`assignment_posts` 키는 그대로 두어
-- 예전 화면이 깨지지 않는다.
...
BEGIN
    ...
    WITH active_roster AS MATERIALIZED (
        ...
        SELECT
            ...
            CASE WHEN p.writing_context = 'self'
                 THEN COALESCE(p.self_writing_type, 'free')
        ...
        SELECT
            ...
            count(*) FILTER (WHERE p.post_type = 'diary')::INTEGER AS diaries,
