...
-- 교사용 학급 운영 현황 1차
--
-- 목록 전체를 브라우저로 내려받아 계산하지 않고 학급/기간별 핵심 활동, 교사가
-- 확인할 일, 최근 미션 현황을 한 번에 반환한다. 글 원문과 댓글 내용은 반환하지
-- 않으며 모든 조회는 class_id를 직접 기준으로 한다.
-- ============================================================================
        ...
        'actions', jsonb_build_object(
            ...
            'reading_pending', jsonb_build_object(
                'count', (
                    ...
                    FROM public.student_posts p
                    JOIN active_roster r ON r.id = p.student_id
                    LEFT JOIN public.reading_log_teacher_reviews review ON review.post_id = p.id
                    WHERE p.class_id = p_class_id
