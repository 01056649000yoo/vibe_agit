-- 식단표가 급식 이름을 2열·3열로 세울 수 있게 저장 검증에 `columns`를 더한다.
--
-- 화면과 서버가 같은 값을 써야 하므로 허용 목록은 `meal-board/mealColumns.js`와 짝이다.
-- 나머지 검증은 20261226에서 그대로 옮겨 오고 식단표 부분만 새로 넣는다.
-- 값이 없는 기존 스크린은 지금까지처럼 2열로 본다(저장도 그대로 통과한다).
