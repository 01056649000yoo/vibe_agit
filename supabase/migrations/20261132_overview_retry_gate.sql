-- 지도가 재도전 관문 상태를 받도록 개요 RPC 에 실어 보낸다.
-- 화면이 스스로 판단하지 않고 서버가 준 값을 그대로 그린다(자격 계산과 같은 원칙).
    ...
    RETURN jsonb_build_object(
        ...
        'summit', v_summit || jsonb_build_object(
            'retry', CASE
                ...
                ELSE public.learning_engine_retry_gate_v1(
                    v_student_id, v_class_id, 'vocab',
                    public.vocab_tower_v2_summit_key(v_grade, (v_summit->>'next_stage')::SMALLINT), 'summit')
            END),
        'decks', v_decks,
