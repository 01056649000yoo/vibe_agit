...
-- 교사용 학급 최근 활동
--
-- 선생님 과제, 학생 자율 독서록, 승인된 학생 댓글을 학급 단위의 한 피드로
-- 반환한다. 목록에는 본문 전체를 싣지 않고 화면에서 선택한 글만 별도 조회한다.
-- ============================================================================

BEGIN;

CREATE INDEX IF NOT EXISTS idx_post_comments_class_approved_created
    ON public.post_comments (class_id, created_at DESC, id)
...
REVOKE ALL ON FUNCTION public.get_class_recent_activity(UUID, TEXT, INTEGER, INTEGER)
FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_class_recent_activity(UUID, TEXT, INTEGER, INTEGER)
