...
/*
 * 2026-08-24: 교사 화면이 `뜻의 방 → 문장의 방 → 구별의 방` 을 현재 흐름처럼 설명하고 있었다.
 * 그 방 표시는 V1 화면에서만 나오고(`{!isV2 && ...}`, V2 학생은 `deck-map` 으로 바로 간다),
 * 전체 학급이 V2 인 지금 **학생은 방을 아예 보지 않는다**. 교사 화면만 옛 구조를 설명한 것이다.
 *
 * ⚠️ 설명은 화면에 있고 출제 규칙은 DB 함수에 있어서, 한쪽만 고치면 다시 어긋난다.
 *    이 검사는 교사 화면이 **실제 출제하는 형태 이름**을 쓰는지, 그리고 연습에만 있고 시험에는 없는
...
test('어휘의 탑 교사 화면은 실제 출제 형태를 설명한다', async () => {
    ...
    assert.match(source, /덜 익힌 낱말 5 · 복습할 낱말 4 · 처음 보는 낱말 3/);

    // 시험은 연습과 출제가 다르다. 특히 쓰임 구별을 내지 않는다는 점을 적어 둔다.
