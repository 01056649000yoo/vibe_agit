...
test('글 종류 첫 화면은 작성 방식별 두 구역과 PC 4열로 전체 선택지를 보여준다', () => {
    assert.match(missionTypePicker, /getGenreCategories\(\)\.flatMap/);
    assert.match(missionTypePicker, /choices\.filter\(\(entry\) => entry\.missionTypeId\)/);
    assert.match(missionTypePicker, /choices\.filter\(\(entry\) => !entry\.missionTypeId\)/);
    assert.match(missionTypePicker, />\s*양식이 준비된 글\s*</);
    assert.match(missionTypePicker, />\s*기본 글쓰기\s*</);
    assert.match(missionTypePicker, /repeat\(4, minmax\(0, 1fr\)\)/);
    ...
    assert.doesNotMatch(missionTypePicker, /repeat\((?:5|6), minmax\(0, 1fr\)\)/);
    assert.doesNotMatch(missionTypePicker, /repeat\(auto-fit/);
});
