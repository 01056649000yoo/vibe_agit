...
BEGIN
    ...
    PERFORM public.record_policy_consent_v1('2026-09-14', 'reconsent');
    PERFORM public.record_policy_consent_v1('2026-09-14', 'reconsent');   -- 두 번 눌러도 한 줄
    ...
    END IF;
    IF (SELECT count(*) FROM public.policy_consents WHERE user_id = v_teacher AND policy_version = '2026-09-14') <> 1 THEN
        RAISE EXCEPTION '같은 판 동의가 중복으로 쌓였습니다.';
    END IF;
    IF NOT (v_result->'agreed_versions') ? '2026-09-14' THEN
    ...
    PERFORM set_config('role', 'authenticated', true);
    BEGIN
        PERFORM public.record_policy_consent_v1('2026-09-14', 'backfill');
