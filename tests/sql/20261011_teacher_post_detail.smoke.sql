...
BEGIN
  ...
  IF v_result->>'version' NOT IN ('1','2') OR v_result->'post'->>'id'<>v_post::text OR v_result->'reactions' IS NULL OR v_result->'comments' IS NULL THEN RAISE EXCEPTION '교사 글 상세 계약 오류: %',v_result; END IF;
