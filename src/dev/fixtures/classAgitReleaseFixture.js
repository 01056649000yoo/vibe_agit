...
export async function createClassAgitReleaseFixture() {
    ...
    const api = {
        ...
        async bookAction(_class, action, p) {
            if (action === 'create') { if (!books.has(p.book_id)) books.set(p.book_id, { id: p.book_id, class_id: previewClass.id, title: '우리 반의 이야기', subtitle: '', introduction: '', class_label: '', term: '', issue_date: '2026-09-05', grouping: 'custom', paper_format: 'A4', design_id: 'botanical', revision: 1, archived: false, items: [], editions: [], history: new Map() }); return workspace(p.book_id); }
