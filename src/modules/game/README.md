...
드래곤과 어휘의 탑은 게임별 `teacherEntry`를 통해 레지스트리에서 지연 로딩한다. 드래곤 교사 화면은
`dragon/TeacherManager.jsx`와 전용 시즌 RPC로 이전했다. 어휘의 탑만 기존 운영 DB 계약의 회귀 위험을 줄이기 위해
당분간 `legacy/LegacyGameManager.jsx` 어댑터를 사용한다. 학생 진입은 별도 검증 후 표준 호스트 계약으로 옮긴다.
