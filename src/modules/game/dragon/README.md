...
수호룡 시즌은 순위 경쟁이나 성장 초기화가 아니라 **학급 운영 기록 구간**이다.

- 전용 테이블 `dragon_growth_seasons`를 사용한다. `agit_season_history`는 아지트 온 클래스 기록이므로 섞지 않는다.
- 시즌 종료는 `close_teacher_dragon_growth_season()`만 사용한다. 종료 시 학생별 성장·이번 시즌 글을 스냅샷으로 보관한다.
- 다음 시즌을 시작해도 `students.pet_data`, 포인트, 선택한 종, 보유·장착 소품, 작가·독자 누적은 수정하지 않는다.
