...
export default function StudentBooks({ route, onNavigate, onReplace, onBack, api = classAgitReleaseApi }) {
    ...
    return <main className="class-agit class-agit-student"><header className="class-agit-gallery__header"><StudentBackButton onClick={onBack} /><button type="button" disabled={state.loading} onClick={() => setRefresh((v) => v + 1)}>문집 다시 확인</button></header>
        ...
        {route.mode === 'books' && data && <><div className="class-agit-student-exhibitions">{data.books.map((book) => <button type="button" className="class-agit-book-cover" key={book.id} onClick={() => navigate({ mode: 'book', editionId: book.id })}><small>{book.number}판</small><h2>{book.title}</h2><p>{book.subtitle}</p><span aria-hidden="true">✦</span><strong>책 펼치기 ↗</strong></button>)}</div>{!data.books.length && <p className="class-agit-empty">아직 서가에 문집이 없어요. 함께 쓴 책이 곧 찾아올 거예요.</p>}</>}
        {route.mode === 'book' && data && <><p>{data.book.subtitle}</p><div className="class-agit-book-introduction">{data.book.introduction}</div><h2>차례 · {data.works.length}편</h2><ol className="class-agit-book-items">{data.works.map((work) => <li key={work.id}><button type="button" onClick={() => navigate({ mode: 'chapter', editionId, workId: work.id })}>{work.title} · {work.author}</button></li>)}</ol>{!data.works.length && <p>지금 읽을 수 있는 작품이 없어요.</p>}<p>{data.book.class_label} · {data.book.term} · {data.book.issue_date} · {data.number}판</p></>}
