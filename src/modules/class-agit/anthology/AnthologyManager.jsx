...
export default function AnthologyManager({ activeClass, api = classAgitReleaseApi, sourceApi = classAgitApi, onExit }) {
    ...
    return <section className="class-agit class-agit-management class-agit-books">
        <header className="class-agit-project-heading"><div><span className="class-agit-eyebrow">우리반 아지트 · 학급 문집</span><h1>{book ? book.title : '우리 반의 책 만들기'}</h1></div>
            <div className="class-agit-header-actions"><TeacherGuideButton tabId="class-agit-books" variant="help" />{(book || onExit) && <Button variant="outline" type="button" disabled={busy} onClick={() => leave(() => { if (book) { setBook(null); setPicker(false); setProjects(null); } else onExit(); })}>{book ? '문집 목록' : '전시 관리로'}</Button>}</div></header>
        ...
        {book && <>
            ...
            <fieldset className="class-agit-design-picker" disabled={busy || book.archived}><legend>판형 · 실제 출력 크기</legend><div className="class-agit-paper-options">
            ...
            </div><p className="class-agit-canvas-caption">판형에 맞춰 여백과 쪽 나눔을 조정합니다. 본문은 12pt, 시는 14pt를 유지합니다. 인쇄할 때 같은 용지 크기와 실제 크기(100%)를 선택해 주세요.</p></fieldset>
            <DesignPicker label="문집 디자인" type="book" options={BOOK_DESIGNS} value={getBookDesign(book.design_id).id} onChange={(design_id) => edit({ ...book, design_id })} disabled={busy || book.archived} />
            ...
            <div className="class-agit-header-actions"><Button variant="primary" type="button" disabled={busy || !dirty || book.archived} onClick={() => run(async () => { receive(await api.saveBook(classId, book)); setMessage('문집 초안을 저장했습니다.'); })}>문집 초안 저장</Button>
                ...
                <Button variant="outline" type="button" disabled={busy || dirty} onClick={() => act(book.archived ? 'restore' : 'archive')}>{book.archived ? '문집 복원' : '문집 보관'}</Button>
                <Button variant="ghost" type="button" disabled={busy} onClick={() => act('delete')}>문집 삭제</Button></div>
