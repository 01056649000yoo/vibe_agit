...
export default function AnthologyManager({ activeClass, api = classAgitReleaseApi, sourceApi = classAgitApi, onExit }) {
    ...
    return <section className="class-agit class-agit-management class-agit-books">
        ...
        {book && <>
            ...
            {picker && <SourcePicker key={`${classId}:${book.id}`} items={book.items} classId={classId} api={sourceApi} onClose={() => setPicker(false)} onAdd={(values) => { const next = addBookItems(book, values.map((value) => bookItemFromSource(value, classId, true))); edit({ ...next, items: sortBookItems(next.items, next.grouping) }); }} />}
