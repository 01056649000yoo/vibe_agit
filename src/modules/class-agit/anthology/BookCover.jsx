...
import { bookCoverKicker } from './contract.js';
...
export default function BookCover({ book }) {
    ...
    return <figure className="class-agit-book-preview"><div className="class-agit-book-cover" data-design={design.id} style={bookCoverStyle(design.id, paper.id)} aria-label="문집 표지 미리보기">
        {bookCoverKicker(book) && <span>{bookCoverKicker(book)}</span>}<h2>{book.title}</h2><p>{book.subtitle}</p><div className="class-agit-cover-mark" aria-hidden="true">{design.mark}</div>{personal && <strong>{book.owner_student_name} 지음</strong>}<p>{book.class_label}</p><small>{book.issue_date}</small>
