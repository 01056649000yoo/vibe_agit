...
## 외부 공유 절차 보완 (2026-09-05 배포 마감)

- 작품별 외부 포함/가림 이름 입력을 없애고 전체 작품을 한 번에 발행한다. `public/sharingPolicy.js`가
  작성자 표시를 작품 순번에서 생성하며 기존 별명·등록 이름을 사용하지 않는다. 읽기 전용 미리보기와 발행에
  같은 표시를 사용하고 부적격 작품이 있으면 일부만 공개하지 않고 전체 발행을 막는다.
- 기존 발행본은 자동으로 교체하지 않는다. 새 발행/공개본 갱신에서 적용하며 기존 원글 검증·기간·철회 RPC를 사용한다.
- `gallery/rightsNotice.js`를 교사 미리보기·학생/외부 입구와 개인정보처리방침·약관이 함께 사용한다.
  입장 버튼만 `확인하고 입장하기`로 바꾸며 추가 동의 기록·브라우저 저장소·조회는 없다.
- 개인정보 안내는 실제 공개 항목, 제목/본문 자동 익명화 없음, 최대 30일 접근 차단과 삭제의 차이,
  중단 요청 경로를 설명한다. 작품 저작권은 작성자 등 해당 저작자에게 있고 법령상 허용되는 이용은 제외한다.
  문구 검토 참고: [한국저작권위원회](https://www.copyright.or.kr/information-materials/common-sense/knowledge-for-netizen/index.do),
  [개인정보보호위원회 처리방침 작성지침 안내](https://m.pipc.go.kr/np/cop/bbs/selectBoardArticle.do?bbsId=BS217&mCode=G010030020&nttId=11134).
...
배포 순서는 SQL → 외부 Edge → 새 앱이다. 사용자 배포 요청으로 SQL `61247`(290/290)과 외부 Edge를 먼저 반영했다. main 푸시로 새 앱을 자동 배포한다.
운영 진남초 4학년 1반 제한과 외부 공유 OFF를 그대로 유지한다. `41편 주제 전시 준비` 샘플로 흐름을 재현할 수 있다.
