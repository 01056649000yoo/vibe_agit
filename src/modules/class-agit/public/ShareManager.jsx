...
function ShareEditor({ data, api, classId, exhibitionId, onClose, onReload, onOpenPublic, archived }) {
    ...
    const changeItem = (id, change) => { setItems((values) => values.map((item) => item.itemId === id ? { ...item, ...change } : item)); setConfirmed(false); retryToken.current = null; };
    if (preview) return <PublicGallery api={preview} token="preview" isPreview onExit={() => setPreview(null)} />;
    return <section className="class-agit"><header className="class-agit-project-heading"><h1>외부 읽기 전용 공유</h1><button type="button" disabled={busy} onClick={onClose}>전시 편집으로</button></header>
        ...
        <fieldset disabled={busy} className="class-agit-book-settings"><legend>외부 공개본</legend><label>외부 전시 제목<input value={title} maxLength={80} onChange={(e) => { setTitle(e.target.value); setConfirmed(false); retryToken.current = null; }} /></label><label>소개<textarea value={introduction} maxLength={240} onChange={(e) => { setIntroduction(e.target.value); setConfirmed(false); retryToken.current = null; }} /></label><label>공유 기간<select value={days} onChange={(e) => { setDays(Number(e.target.value)); retryToken.current = null; }}>{[1, 7, 14, 30].map((day) => <option key={day} value={day}>{day}일</option>)}</select></label></fieldset>
        ...
        <div className="class-agit-header-actions"><button type="button" disabled={!selected.length || busy} onClick={() => setPreview(createPublicPreviewApi({ title, introduction, works: selected.map((item, index) => ({ id: `published-${index + 1}`, title: item.title, author: item.publicAlias, format: item.format, kindLabel: item.kindLabel, excerpt: item.excerpt, blocks: item.blocks })) }))}>외부 방문자 미리보기</button><button type="button" disabled={busy} onClick={onReload}>최신 공유 설정 불러오기</button></div>
        <label className="class-agit-confirmation"><input type="checkbox" checked={confirmed} disabled={busy} onChange={(e) => setConfirmed(e.target.checked)} />제목·본문·가림 이름·소개와 외부 공개 의사를 확인했습니다.</label>
        <button type="button" className="class-agit-primary" disabled={busy || archived || !confirmed || !selected.length || !title.trim() || !data.external_enabled} onClick={() => run('publish')}>{current.share ? '공개본 갱신 · 새 주소 발급' : '외부 공유 주소 만들기'}</button>
        {url && <div className="class-agit-share-link"><label>새 공유 주소<input aria-label="새 공유 주소" readOnly value={url} /></label><button type="button" onClick={async () => { try { await navigator.clipboard.writeText(url); setMessage('공유 주소를 복사했습니다.'); } catch { setMessage('주소 칸을 선택해 직접 복사해 주세요.'); } }}>주소 복사</button><a href={url} target="_blank" rel="noopener noreferrer" onClick={onOpenPublic ? (event) => { event.preventDefault(); onOpenPublic(new URL(url).hash.slice(1)); } : undefined}>방문자로 열기 ↗</a></div>}
        {current.share && <><h2>발행한 외부 공개본</h2><p>{current.share.publication_no}판 · {current.share.revoked ? '해지됨' : current.share.expired ? '만료됨' : '공유 중'} · 만료 {new Date(current.share.expires_at).toLocaleString('ko-KR')}</p>
            <p>학생 홈 공개 스위치와 외부 공유는 별개입니다. 주소는 원문으로 보관하지 않아 이 화면을 다시 열면 재발급해야 합니다.</p><div className="class-agit-header-actions"><button type="button" disabled={busy || current.share.revoked || !data.external_enabled} onClick={() => run('rotate')}>주소 재발급</button><button type="button" disabled={busy || current.share.revoked || !data.external_enabled} onClick={() => run('extend')}>선택한 기간으로 만료 변경</button><button type="button" disabled={busy || current.share.revoked} onClick={() => run('revoke')}>공유 주소 해지</button></div>
            <ul className="class-agit-projects">{current.published_items.map((item) => <li key={item.id}><span>{item.title} · {item.author}{item.revoked ? ' · 철회됨' : ''}</span><button type="button" disabled={busy || item.revoked} onClick={() => run('withdraw', { item_id: item.id })}>이 작품 외부 수록 철회</button></li>)}</ul></>}
        {confirmDialog}
    </section>;
}
export default function ShareManager({ classId, exhibitionId, api = classAgitReleaseApi, onClose, onOpenPublic, archived = false }) {
    ...
    if (error) return <div className="class-agit"><p role="alert">{error}</p><button type="button" onClick={() => setVersion((v) => v + 1)}>공유 다시 불러오기</button><button type="button" onClick={onClose}>전시 편집으로</button></div>;
    return data ? <ShareEditor key={data.loadedVersion} data={data} api={api} classId={classId} exhibitionId={exhibitionId} onClose={onClose} onOpenPublic={onOpenPublic} archived={archived} onReload={() => setVersion((v) => v + 1)} /> : <p role="status">공유 설정을 불러오고 있습니다…</p>;
}
