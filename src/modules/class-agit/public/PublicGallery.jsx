...
export default function PublicGallery({ token, api = publicClassAgitApi, isPreview = false, onExit }) {
    ...
    const activeRoom = data?.rooms.find((entry) => entry.number === room);
    const openWork = (work) => {
    ...
    return <main ref={root} className="class-agit class-agit-public">
        ...
        {data && <><h1 tabIndex={-1}>{data.title}</h1>{room === 0 ? <div className="class-agit-lobby"><div><p>{data.introduction}</p><p>{data.total_count}편의 이야기 · {data.rooms.length}개의 전시실</p><ExhibitionRightsNotice /><nav className="class-agit-room-nav" aria-label="주제별 입장">{data.rooms.map((entry) => <button type="button" key={entry.number} onClick={() => go({ room: entry.number, workId: null, view: 'room' })}>{entry.title || `${entry.number} 전시실`} · {entry.count}편 ↗</button>)}</nav><button type="button" className="class-agit-primary" disabled={!data.total_count} onClick={() => go({ room: data.rooms[0]?.number, workId: null, view: 'room' })}>{EXHIBITION_RIGHTS.enter}</button></div><div className="class-agit-lobby-art" aria-hidden="true"><span>✦</span><p>작은 글이 모여<br />우리의 이야기가 됩니다.</p></div></div> : <>
            ...
            <nav className="class-agit-room-nav" aria-label="전시실 이동">{data.rooms.map((entry) => <button type="button" key={entry.number} aria-current={room === entry.number ? 'page' : undefined} onClick={() => go({ room: entry.number, workId: null, view: route.view }, true)}>{entry.title || `${entry.number} 전시실`} · {entry.count}편</button>)}</nav>
            <h2>{activeRoom?.title}</h2><p>{activeRoom?.introduction}</p>
            {route.view === 'room' ? <GalleryRoom theme={data.theme} variant={activeRoom?.variant} roomTitle={activeRoom?.title} key={room} roomNumber={room} works={data.items} onOpen={openWork} /> : <ol className="class-agit-work-list">{data.items.map((work) => <li key={work.id}><button type="button" data-work-id={work.id} onClick={() => openWork(work)}><div><strong>{work.title}</strong><p>{work.excerpt}</p><small>{work.author}</small></div></button></li>)}</ol>}
