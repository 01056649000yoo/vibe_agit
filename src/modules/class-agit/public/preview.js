...
export function createPublicPreviewApi({ title, introduction, works, theme = 'garden' }) {
    return { async read(_token, room = 0, workId = null) {
        ...
        return { version: 1, title, introduction, theme, publication_no: 1, room, rooms, total_count: works.length, items, work: workId ? works.find((work) => work.id === workId) : null };
    } };
}
