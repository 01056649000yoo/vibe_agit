...
export default function ClassAgitTeacherEntry({ activeClass, api = classAgitApi, isSample = false, releaseApi = classAgitReleaseApi, isAdmin = false, onOpenPublic }) {
    ...
    if (area === 'share' && workspace?.draft) return <ShareManager classId={classId} exhibitionId={workspace.draft.id} api={releaseApi} archived={workspace.draft.state === 'archived'} onOpenPublic={onOpenPublic} onClose={() => setArea('exhibitions')} />;
    if (publication && workspace?.draft) return <PublishedExhibition classId={classId} exhibitionId={workspace.draft.id} api={api} onExit={() => setPublication(false)} />;
    return <div>
        <div className="class-agit-live-access">
        ...
        {!workspace?.draft && releasesEnabled && <div className="class-agit-header-actions"><button type="button" onClick={() => setArea('books')}>학급 문집 만들기</button>{isAdmin && <button type="button" onClick={() => setArea('rollout')}>공개 단계 관리</button>}</div>}
        {error && <div className="class-agit-error" role="alert">{error}{!workspace?.draft && <button type="button" className="class-agit-secondary" onClick={() => setLoadVersion((version) => version + 1)}>작업공간 다시 불러오기</button>}</div>}
        ...
        {workspace?.draft ? <ExhibitionWorkbench key={workspace.draft.id} activeClass={activeClass} initialDraft={workspace.draft}
            students={workspace.students} sources={candidates.items} persistence={{
                busy, isSample, moduleEnabled: workspace.class.module_enabled,
                ...
                exit: () => setWorkspace((data) => ({ ...data, draft: null })), openPublication: () => setPublication(true), openShare: releasesEnabled ? () => setArea('share') : undefined,
            }} /> : workspace && <section className="class-agit">
                <div className="class-agit-project-heading"><div><span className="class-agit-eyebrow">{activeClass.name}</span><h1>우리 반의 글 전시</h1><p>초안을 준비하고 확인한 내용으로 학급 공개판을 만듭니다.</p></div>
                    <div className="class-agit-header-actions"><TeacherGuideButton tabId="class-agit" variant="help" /><button type="button" className="class-agit-primary" disabled={busy || workspace.projects.length >= 20} onClick={create}>새 전시 만들기</button></div></div>
                ...
                <ul className="class-agit-projects">{workspace.projects.map((project) => <li key={project.id}><div><strong>{project.title}</strong><p>{project.state === 'published' ? `${project.publication_no}판 공개 중` : project.state === 'archived' ? '보관함' : '비공개 초안'}</p></div><button type="button" className="class-agit-secondary" disabled={busy} onClick={() => open(project.id)}>전시 열기</button></li>)}</ul>
