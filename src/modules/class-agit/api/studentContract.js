import { GALLERY_THEMES } from '../designs.js';
const validTheme = (theme) => theme === undefined || GALLERY_THEMES.some((item) => item.id === theme);
...
export function assertStudentExhibitions(data) {
    if (!keys(data, ['version', 'exhibitions']) || data?.version !== 1 || !Array.isArray(data.exhibitions) || data.exhibitions.length > 20
        || data.exhibitions.some((item) => !record(item) || !text(item.id, 36) || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(item.id) || !text(item.title, 80) || !text(item.introduction, 240)
            || !validTheme(item.theme) || !Number.isInteger(item.publication_no) || item.publication_no < 1
            || Object.keys(item).some((key) => !['id', 'title', 'introduction', 'publication_no', 'published_at', 'theme'].includes(key)))) fail();
    return data;
}

export function assertStudentRoom(data, id, room) {
    if (!keys(data, ['version', 'exhibition_id', 'publication_no', 'title', 'introduction', 'room', 'rooms', 'total_count', 'items', 'visibility_revision', 'theme']) || data?.version !== 1 || !validTheme(data.theme) || data.exhibition_id !== id || data.room !== room || !text(data.title, 80) || !text(data.introduction, 240)
