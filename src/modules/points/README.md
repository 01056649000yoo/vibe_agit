...
댓글 활동은 포인트와 분리해 승인된 친구 글 댓글·반응을 **소통 칭호 성장**으로 인정한다. 2026-08-26까지
지급된 `comment_reward` 원장과 잔액은 이전 기록으로 보존하지만 새 댓글 보상은 만들지 않는다.
독서록·일기·어휘의 탑·수호룡 구매도 기능 전용 RPC 한 번으로 동작하며, 아직 직접 원장을 쓰는 부분은 다음 변경 때
`point_engine_apply()`로 순차 이전한다.
