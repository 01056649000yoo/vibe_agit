...
const TeacherSettingsTab = ({
    ...
    handleSaveTeacherSettings, handleTestAIConnection, runAIDiagnosis
}) => {
    const [activePromptTab, setActivePromptTab] = useState('feedback');
    const isFeedback = activePromptTab === 'feedback';

    return (
        <section style={{ maxWidth: '920px', margin: '0 auto', width: '100%', background: 'white', borderRadius: '24px', padding: isMobile ? '20px' : '32px', border: '1px solid #DCE6EE', boxShadow: '0 4px 18px rgba(15, 23, 42, 0.04)', boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '20px', alignItems: isMobile ? 'flex-start' : 'center', flexDirection: isMobile ? 'column' : 'row', marginBottom: '24px' }}>
                <div>
            ...
            <textarea
                ...
                style={{ width: '100%', minHeight: '340px', padding: '16px', borderRadius: '14px', border: '1px solid #CBD5E1', background: '#F8FAFC', fontSize: '0.94rem', lineHeight: 1.7, color: '#1E293B', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
            />
