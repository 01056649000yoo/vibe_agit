...
const TeacherWritingHub = ({ activeClass, isMobile, setSelectedActivityPost }) => {
    ...
    return (
        <div style={{ width: '100%' }}>
            <div
                role="tablist"
                aria-label="글쓰기 관리 종류"
                style={{
                    display: 'flex', gap: '10px', marginBottom: '24px', padding: '6px',
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            style={{
                                minWidth: isMobile ? '180px' : '220px', padding: '12px 18px',
                                border: active ? '1px solid #BFDBFE' : '1px solid transparent',
                                borderRadius: '14px', background: active ? 'white' : 'transparent',
                                boxShadow: active ? '0 5px 16px rgba(30, 64, 175, 0.10)' : 'none',
                                color: active ? '#1D4ED8' : '#64748B', cursor: 'pointer', textAlign: 'left'
                            }}
                        >
                            <strong style={{ display: 'block', fontSize: '0.98rem' }}>{tab.label}</strong>
                            <small style={{ display: 'block', marginTop: '4px', color: active ? '#60A5FA' : '#94A3B8' }}>
                                {tab.description}
                            </small>
                        </button>
                    );
