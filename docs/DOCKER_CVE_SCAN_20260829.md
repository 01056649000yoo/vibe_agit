...
- 원본 JSON은 개인정보·비밀을 포함하지 않고 호스트의 서비스 검사 보관 경로에 gzip·권한 600으로 둔다.
  DB와 브라우저에는 원본 대신 제한된 요약과 SHA-256만 기록한다.

## 정기 운영 전환

2026-08-29에 관리자 대시보드 `운영 → 서비스 관리` 원장과 월간 검사기를 추가했다. 첫 기준 검사는 다음 날
Supabase 업데이트 확인 후 사용자가 첫 점검을 요청할 때 실행하며, 그 성공 시각부터 30일 간격으로 검사한다.
대시보드는 원본 JSON이 아니라 이미지별 digest·노출 등급과 CRITICAL/HIGH/수정 가능/긴급 개수만 표시한다.
전체 절차는 [서비스 관리 운영 문서](SERVICE_MANAGEMENT.md)를 따른다.
