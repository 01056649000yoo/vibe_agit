...
기존 5분 건강검진과 별도로 판정 기준을 만들지 않는다. 맥미니의
`scripts/report-service-health.sh`가 건강검진을 한 번 실행하고 현재 앱·DB와 열린 운영 경고를 확인한 뒤
다음 둘 중 한 줄만 출력한다.
...
OpenClaw command cron은 AI 모델을 호출하지 않고 위 스크립트의 stdout 한 줄을 현재 연결된 텔레그램
...
즉시 실행을 한 번 해 실제 텔레그램에 한 줄만 도착하는지 확인한다. 중지할 때는 해당 cron ID를 찾아
