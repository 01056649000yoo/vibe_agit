...
> **진행 상황**: 자비스·샘링크 이관과 옛 `supabase` 컨테이너 14개 제거를
> **2026-08-28 완료**했다. 아래 조사·이관 절차는 과거 기록으로 남긴다.
> 이전하며 배운 것은 아래 `실제로 해 보니` 절에 적었다.
>
> **2026-08-29 후속 정리**: 오늘 통합 백업으로 앱별 복구 3/3을 실제 확인한 뒤 옛 PG15 이미지·
> 미사용 소형 볼륨·297MB bind 데이터를 제거하고 기본 Compose 파일을 `retired` 처리했다. 이관 통합
> 덤프는 별도 보존하며, 현재 Jarvis Caddy 등 4개가 쓰는 `supabase_default` 네트워크는 유지한다.
...
- 옛 스택 14개를 보다 최신 통합 DB 로컬 덤프(15MB)와 이관 백업을 남기고 제거했다.
- `supabase.샘링크.kr`과 샘링크의 예전 `/supabase/*` 경로는 통합 Kong으로 보낸다.
- `supabase_default` 네트워크는 Jarvis·샘링크·서바이벌의 현재 컨테이너 별칭 호환 때문에 유지한다.
- 일일 백업은 `agit-db`의 `app`·`samlink`까지 포함하고, 샘링크 보조 백업도 `agit-db.samlink`를 본다.
