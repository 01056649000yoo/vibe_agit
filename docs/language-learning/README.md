...
- **아직 학생에게 제공하지 않는다.** 교육과정 기준과 실제 제공 가능 학년군은 분류했지만 모든 항목과
  원본 문제는 여전히 `source_imported`다. 표현·뜻과 선택형 확인 문제를 직접 검수하기 전에는 출제하지 않는다.

파일명의 `source-import-v1`은 옛 앱에서 가져온 첫 원본 묶음을 뜻한다. 내부 `schemaVersion: 2`는 그 원본에
교육과정 기준과 제공 학년 분류를 덧붙인 현재 데이터 계약 버전이다.
...
현재 첫 직접 검수 대상으로 속담 20개와 사자성어 20개를 `core-v1` 시범팩에 골랐다. 이 중 별표 항목이
3·4학년 미리 만나기 20개이며, 시범팩 밖 145개는 아직 내용 난이도를 지정하지 않았다.
...
1. 선정한 시범 40개의 표현·뜻·한자·예문과 `contentLevel`을 직접 검수한다.
2. 3·4학년 미리 만나기 20개에는 쉬운 `meaningChoice`, 시범 40개에는 5·6학년용 확인 문제와 설명을
   작성하고 검수 완료 팩을 DB에 승격한다.
