...
- 최초 정상화는 **2026-10-03~05 개천절 3일 연휴**에 진행한다. 9월 25일에 당시 공식 릴리스 중 14일 이상
  관찰된 최신 호환 태그를 고정하고, Kong override를 유지한다. v0.7.2는 현재 확인된 최소 정상화 기준점이며
  10월 실제 도착 태그는 그때의 공식 변경 기록을 다시 읽고 확정한다.
...
v0.7.2는 2026-08-29 기준 필요한 서비스 이미지들을 포함하면서 Kong을 기본 게이트웨이로 유지한다. 현재
Caddy, 배포 스크립트, 건강검진, 통합 연구소가 `agit-kong` 또는 8100을 참조하므로 첫 작업에서 게이트웨이
구조까지 바꾸지 않는다. 10월에는 v0.7.2를 무조건 설치하는 대신 이 버전부터 후보 태그까지의 변경을 단계별
dry-run하고, 운영에는 9월 25일에 고정한 한 묶음만 반영한다.
[v0.7.2 공식 Compose](https://github.com/supabase/supabase/blob/self-hosted/v0.7.2/docker/docker-compose.yml)

### 10월 운영 도착 태그: 최신 호환 묶음 + Kong override

2026-08-29의 최신 공식 묶음은 `self-hosted/v0.8.0`이다. 이 릴리스는 Envoy를 기본 게이트웨이로 바꾸지만
공식 Kong override를 계속 제공한다. 10월 운영에는 v0.7.2와 이후 릴리스를 여러 번 순차 재기동하지 않고,
9월 25일에 고정한 최신 호환 태그 한 개만 반영한다. v0.7.2는 혼합 설치의 변경점을 읽고 dry-run하는 중간
기준으로 사용한다.
...
2. **격리 사본 만들기**: 현재 세 Compose 파일과 비밀을 제외한 설정을 복제하고, 최신 백업을 임시 DB·Storage에
   실제 복원한다.
3. **단계별 dry-run**: 정한 base에서 v0.7.2를 거쳐 9월 25일에 고정한 후보까지의 파괴적 변경을 순서대로
   검토한다. 특히 v0.7.0의 `API_EXTERNAL_URL`·노출 스키마·Realtime 암호화 키를 확인한다.
...
| 2026-08-30~09-25 | 기존 백업 7일 관측과 9월 복구 확인, 현재 base 조사, 후보 릴리스 변경 기록 검토 |
| 2026-09-25 | 9월 19일까지 공개돼 14일 이상 관찰된 최신 호환 태그를 후보로 고정 |
| 2026-09-26~10-02 | 격리 복원·단계별 dry-run·세 앱/연구소 스모크·CVE 재검사 |
| 2026-10-01 04:40 이후 | 월간 실제 복구 3/3을 최종 운영 게이트로 확인 |
| 2026-10-03 | 연휴 1일차: 설정 diff·백업·롤백 자산·이미지 pull 최종 확인 |
| 2026-10-04 00:00~02:00 | 연휴 2일차: 고정한 공식 묶음 + Kong override 운영 반영 |
| 2026-10-04~05 | 연휴 남은 기간: 집중 관찰·필요 시 롤백, 이전 이미지 보존 |
