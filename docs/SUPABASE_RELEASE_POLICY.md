...
- 이번 최초 정상화는 바로 최신 `self-hosted/v0.8.0`으로 건너뛰지 않고,
  **`self-hosted/v0.7.2` + Kong을 1차 기준점**으로 삼는다. 현재 여러 경로가 `agit-kong`과 8100을 직접
  참조하고, v0.8.0은 기본 게이트웨이를 Kong에서 Envoy로 바꾸는 파괴적 변경이기 때문이다.
- 전체 묶음 반영 전이라도 공식 릴리스가 명시한 작은 보안 설정은 따로 선반영할 수 있다. 현재 Kong은
  anon 키 요청에서 `/realtime/v1/api/openapi`가 200이고 `/tenants`만 403이라, 두 관리 경로를 gateway에서
  명시적으로 403 처리하는 v0.6.0 보안 설정을 우선 적용한다.
- 1차 정상화가 7일간 안정적이면 다음 정기 창에 최신 공식 묶음으로 전진하되, 처음에는 공식 Kong override를
  유지한다. **Envoy 전환은 서비스 버전 갱신과 분리한 별도 작업**으로 한다.
...
### 선행 보안 보완: Realtime 관리 경로 차단

2026-08-29 로컬 Kong 실응답을 공개 anon 키로 확인한 결과는 다음과 같다.
...
v0.7.2는 최신 서비스 이미지들을 포함하면서 Kong을 기본 게이트웨이로 유지한다. 현재 Caddy, 배포 스크립트,
건강검진, 통합 연구소가 `agit-kong` 또는 8100을 참조하므로 첫 작업에서 게이트웨이 구조까지 바꾸지 않는다.
[v0.7.2 공식 Compose](https://github.com/supabase/supabase/blob/self-hosted/v0.7.2/docker/docker-compose.yml)

### 2차: 최신 공식 묶음 + Kong override

2026-08-29의 최신 공식 묶음은 `self-hosted/v0.8.0`이다. 이 릴리스는 Envoy를 기본 게이트웨이로 바꾸지만,
공식 Kong override를 계속 제공한다. 1차 적용 뒤 7일 동안 오류·백업·복구가 정상이라면 다음 정기 창에는
당시 최신 릴리스로 올리면서 Kong override를 유지한다.
...
2. **격리 사본 만들기**: 현재 세 Compose 파일과 비밀을 제외한 설정을 복제하고, 최신 백업을 임시 DB·Storage에
   실제 복원한다.
3. **단계별 dry-run**: 정한 base에서 v0.7.2까지의 파괴적 변경을 순서대로 검토한다. 특히 v0.6.0의
   Realtime gateway 보안 설정, v0.7.0의 `API_EXTERNAL_URL`·노출 스키마·Realtime 암호화 키를 확인한다.
...
| 시점 | 할 일 |
|---|---|
| 다음 작은 보안 변경 창 | Realtime `openapi`·`tenants` 관리 경로를 Kong에서 명시적 403 차단 |
| 2026-08-30 05:00 이후 | 토·일 백업 2일치와 관리자 원장 대조 |
| 2026-09-01 04:40 이후 | 월간 실제 복구 3/3 확인 |
| 2026-09-01~05 | base 확정, v0.7.2 격리 복원·dry-run·스모크·CVE 재검사 |
| 2026-09-06 00:00~02:00 | 모든 게이트 통과 시 v0.7.2 + Kong 운영 반영 |
| 반영 후 7일 | 일일 백업·복구·오류·재시작 추이 관찰, 이전 이미지 보존 |
| 다음 정기 창 | 당시 최신 공식 묶음 + Kong override 검토; Envoy 전환은 별도 결정 |
...
|---|---|---|---|---|
| 2026-08-29 | `self-hosted/v0.8.0` | v0.7.2로 먼저 정상화 | 현재 설치 기준점 없음, Kong 직접 참조, v0.8.0 gateway 파괴적 변경 분리 | 2026-08-30 백업, 2026-09-01 복구 |
