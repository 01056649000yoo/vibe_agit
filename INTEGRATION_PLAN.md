...
4. 교사 `설정 → 글쓰기 창 관리`를 연동 제어판으로 사용:
   - 학급별 연구소 결과물 불러오기 ON/OFF
   - 개요짜기·한줄쓰기 등 학생에게 노출할 결과물 유형 선택
   - 연결 상태와 마지막 확인 상태 표시, 실제 공용 입력창 기반 학생 미리보기
   - `writing_editor_settings.enabled_tools`와 글쓰기 도구 매니페스트를 사용해 새 연구소 기능도 같은 방식으로 확장
5. 책임을 분리한다: 학급 공통 설정은 `글쓰기 창 관리`, 특정 과제의 room 연결·질문 선택은 미션 생성 화면,
   연구소 방과 활동 자체 관리는 연구소에서 맡는다. 아지트에는 연구소 관리 코드를 복제하지 않는다.
