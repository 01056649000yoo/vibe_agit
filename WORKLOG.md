...
## 2026-07-28 — UI 작업 6: 학생 명단·최근 활동 PC 2열화 (GPT/Codex)
- **한 일**:
  - 학생 탭에서 `학생 명단 / 최근 활동`을 좁은 좌우 칸으로 나누던 배치를 없애고 두 영역을 위아래 전체 폭 카드로 정리.
  - 학생 명단 내부를 PC 2열 카드 목록으로 변경. 각 카드에 번호·이름·로그인 코드·포인트·복사/포인트/더보기 작업을 유지하면서
    한 학생 행이 화면 전체 너비를 차지하지 않게 함. 모바일은 기존 1열 동선을 유지.
  - 최근 활동도 PC 정확히 2열로 변경하고 카드 최소 높이 70px·영역 높이 440px로 설정해 5줄, 즉 최소 10건이 한 화면에 보이도록 함.
    조회 상한 20건과 DB 계약은 그대로 두고, 활동 카드를 키보드로 누를 수 있는 버튼으로 변경.
- **변경**: `TeacherStudentHub.jsx`, `StudentManagementList.jsx`, `RecentActivity.jsx`의 레이아웃만 변경. DB·운영 인프라 변경 없음.
- **결과/검증**: 변경 파일 ESLint 0에러(기존 `RecentActivity` exhaustive-deps 경고 1건 유지), `git diff --check` 통과,
  프로덕션 빌드 성공. 연결 가능한 브라우저 세션이 없어 로그인 후 실제 폭·버튼 겹침 확인은 미실시.
- **남은 것 / 다음**: 교사 PC에서 ①학생 2열 카드의 작업 버튼 겹침 여부 ②최근 활동 10건이 스크롤 없이 보이는지,
  모바일에서 두 목록이 1열로 쌓이는지 확인 후 간격만 미세 조정.
