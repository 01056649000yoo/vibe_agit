...
- **남은 것 / 다음**: **브라우저 실사용 확인 미실시**(교사 구글 로그인 자동화 불가) —
  ①제출 현황에서 개별/일괄 회수 눌러 안내문·결과 확인 ②회수 후 학생 화면 `선생님이 걷어감` 표시 확인
  ③`handleUndoRecall`은 훅에만 있고 **UI 버튼 미연결** — 되돌리기 UI 필요 시 글 상세(PostDetailViewer)에 추가.
