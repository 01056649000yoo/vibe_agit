"""Check actual PDF pagination, all prose/stanzas, 12pt minimum, and A4 bounds."""
import json
import logging
import re
import sys
...
with pdfplumber.open(sys.argv[1]) as pdf:
    ...
    for number, page in enumerate(pdf.pages, 1):
        assert abs(page.width - 595.28) < 1 and abs(page.height - 841.89) < 1, f'Not A4: {number}'
    ...
    print(f'PASS: 100 works, {len(pdf.pages)} A4 pages, 100 correct TOC pages, no missing text/stanzas, minimum 12pt, no clipping.')
