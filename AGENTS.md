...
| **[TITLE_SEASON_REWARD_PLAN.md](TITLE_SEASON_REWARD_PLAN.md)** | 네 칭호 공용 학기 시즌·기록가/독서가 단계 포인트·직접 수령 구현 순서 |
| **[POINT_ECONOMY_PLAN.md](POINT_ECONOMY_PLAN.md)** | 글쓰기·게임 포인트 예산, 공방 가격, 한 학기 한 세트 운영 목표 |
...
## 반복 수정·마감·배포 규칙 (2026-09-01 갱신)

### 기본은 로컬 반복 수정
- 사용자가 **`배포`·`마무리`·`확정`을 명시하기 전까지** 코드와 문서는 작업트리에만 수정한다.
- 이 단계에서는 바뀐 영역의 전용 검사와 필요한 로컬 미리보기만 실행한다. 매 수정마다 전체 검사·빌드·WORKLOG·
  ROADMAP·커밋·푸시·Actions 확인을 반복하지 않는다.
- 읽기·편집·관련 검사·로컬 개발 서버·로컬 미리보기 같은 안전한 작업은 별도 승인 없이 진행한다.
- 화면 반복 조정은 가능한 경우 `src/dev/`의 DB 없는 개발 전용 미리보기를 우선 사용한다. 운영 배포를 미리보기
  수단으로 쓰지 않는다.

### 다른 컴퓨터로 이어갈 때는 배포 없이 동기화
- 사용자가 **`동기화` 또는 `다른 컴퓨터에서 이어서 작업`을 명시하면**, 현재 로컬 변경에 관련 전용 검사만 실행한
  뒤 `[skip ci]` 커밋으로 `origin/main`에 저장한다. WORKLOG·ROADMAP·전체 검사·운영 배포 확인은 이때 하지 않는다.
- 이 커밋은 코드 보관과 기기 간 인계를 위한 것이다. 다음 컴퓨터에서는 `origin/main`을 받은 뒤 그대로 이어간다.
...
5. 검사가 통과하면 의미 단위로 커밋하고 별도 재확인 없이 `origin/main`에 한 번 푸시해 자동 배포한다. Actions
   ...
   실패하면 배포하지 말고 원인과 남은 일을 보고한다.

> 사용자가 단순히 `수정해 줘`·`적용해 줘`라고 한 것은 로컬 반복 수정 승인이다. `동기화`는 원격 저장 승인일 뿐
> 외부 배포 승인으로 해석하지 않는다.
...
## 보안 변경 규칙 (2026-08-08)
- **보안 점검은 날짜별 파일로 남긴다** (2026-09-24 선생님 결정). [`docs/security-audits/`](docs/security-audits/README.md)에
  `YYYY-MM-DD.md` 한 파일 + 목록 표 맨 위 한 줄. 기준(예: KISA 시큐어코딩 7개 영역)·확인 방법·결과·고친 것·결정 필요를 적고,
  비밀 값은 적지 않는다. 운영 설정 변경은 선생님 확인 뒤에 한다.
